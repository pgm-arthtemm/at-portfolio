<script>
	...
	onMount(() => {
		...
		gsap.to('.mobile-intro', { left: '-110%', duration: 1, delay: start });
...
<div class="hero mt-72 xl:mt-0 w-11/12 md:w-9/12 text-6xl leading-snug font-bold">
	<div class="hidetext">
		<h2 class="hello text-center text-7xl md:text-8xl md:text-left lg:text-9xl">
			{#each wordArray as letter, index}
				{#if index === 5}
					<span
						class="text-red-700 transition duration-300 ease-in-out hover:text-white cursor-pointer"
						>{letter}</span
					>
				{:else}
					<span class="transition duration-300 ease-in-out hover:text-red-700 cursor-pointer"
						>{letter}</span
					>
				{/if}
			{/each}
		...
		<div class="info text-center md:text-left">
			...
			<h2 class="text-3xl md:text-5xl xl:text-6xl mb-6">
				Arthur <span class="text-red-700 text-3xl md:text-5xl xl:text-6xl mb-6">Temmerman</span>
			</h2>
			<Typewriter />
			<div class="about">
				<p
					class="text-lg text-center mb-12 mt-12 md:text-xl md:text-left md:w-2/3 md:leading-10 xl:w-1/3"
				>
					I am an ambitious full stack web developer who is always looking for new tips and tricks
					to make a website look awesome. I am a programming student at Arteveldehogeschool Ghent
				</p>
			</div>
			<Tiles />
		</div>
